...
type NavItem = { id: string; label: string; href?: string; order: number; isExternal?: boolean; active?: boolean; children?: any[] }
...
function readFileNav(): NavItem[] | null {
  try {
    ...
    if (Array.isArray(arr)) return ensureNavItems(arr) as NavItem[]
  } catch {}
  return null
}
...
async function readDbNav(includeInactive: boolean): Promise<NavItem[] | null> {
  try {
    ...
    const items = ensureNavItems(arr) as NavItem[]
...
export async function GET(request: Request) {
  try {
    ...
    const base = ensureNavItems(fileItems && fileItems.length ? fileItems : DEFAULT_NAV_ITEMS)
    const filtered = includeInactive ? base : base.filter(i => i.active !== false)
    return NextResponse.json(filtered.sort((a, b) => Number(a.order || 0) - Number(b.order || 0)))
  } catch {
    return NextResponse.json(ensureNavItems(DEFAULT_NAV_ITEMS), { status: 200 })
  }
}
