    ...
    <div className="space-y-6">
      <Card className="py-12 px-8 text-center space-y-6 relative overflow-hidden bg-gradient-to-br from-white to-slate-50">
        ...
        {(() => {
          ...
          return (
            <>
              {showT && <h1 className={`text-3xl font-bold text-gray-800 ${loading ? 'opacity-0' : ''}`}>{hide ? t : (t || '一站式图像与运营处理工具')}</h1>}
              {showS && <p className={`text-gray-500 ${loading ? 'opacity-0' : ''}`}>{hide ? s : (s || '轻松处理您的数据，提升工作效率')}</p>}
            </>
          )
        })()}
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="h-14 bg-[#5b5bd6] text-white flex items-center px-10 shadow-md z-20">
        <div className={`flex items-center gap-2 font-bold text-lg ${loading ? 'opacity-0' : ''}`}>
