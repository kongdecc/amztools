...
const AmazonCalculator = () => {
  ...
  const [neededReviews, setNeededReviews] = useState(0);
  const [currentScore, setCurrentScore] = useState(0);
  const [totalRatings, setTotalRatings] = useState(0);
  const [currentRatingVal, setCurrentRatingVal] = useState(0);

  // --- State: Sales Estimator (Listing Level) ---
  const [newRatingsCount, setNewRatingsCount] = useState(10);
  const [ratingRateMin, setRatingRateMin] = useState(8.0); // Default 8%
  const [ratingRateMax, setRatingRateMax] = useState(10.0); // Default 10%
  const [salesEstimate, setSalesEstimate] = useState({ min: 0, max: 0 });

  // --- State: Feedback Estimator (Store Level) ---
  const [feedbackCount, setFeedbackCount] = useState(5);
  const [feedbackRateMin, setFeedbackRateMin] = useState(0.6); // Default 0.6% (6‰)
  const [feedbackRateMax, setFeedbackRateMax] = useState(0.8); // Default 0.8% (8‰)
  const [storeSalesEstimate, setStoreSalesEstimate] = useState({ min: 0, max: 0 });

  // Colors
  const COLORS = ['#22c55e', '#84cc16', '#eab308', '#f97316', '#ef4444'];

  // 1. Calculate Ratings Logic
  useEffect(() => {
  ...
  // 2. Calculate Sales Logic (Listing)
  useEffect(() => {
    const rMin = ratingRateMin > 0 ? ratingRateMin / 100 : 0.08;
    const rMax = ratingRateMax > 0 ? ratingRateMax / 100 : 0.10;
  ...
  // 3. Calculate Sales Logic (Store Feedback)
  useEffect(() => {
  ...
  const handleRatingChange = (star, value) => {
    setRatings(prev => ({
      ...prev,
      [star]: parseInt(value) || 0
    }));
  };
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 space-y-6 bg-gray-50 min-h-screen font-sans text-gray-800">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Left Column: Rating Calculator (Span 7) */}
        ...
        {/* Right Column: Sales Estimators (Span 5) */}
        <div className="lg:col-span-5 space-y-6">
          
          {/* 1. Listing Sales Estimator */}
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            ...
            <div className="bg-green-50 p-3 rounded-lg border border-green-100 mb-4">
              ...
              <div className="flex items-center gap-2">
                <input 
                  type="number" 
                  value={ratingRateMin}
                  onChange={(e) => setRatingRateMin(parseFloat(e.target.value))}
                  className="w-full px-2 py-1 text-sm border border-green-200 rounded text-center"
                />
                <span className="text-gray-400">-</span>
                <input 
                  type="number" 
                  value={ratingRateMax}
                  onChange={(e) => setRatingRateMax(parseFloat(e.target.value))}
                  className="w-full px-2 py-1 text-sm border border-green-200 rounded text-center"
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="number"
                  value={newRatingsCount}
                  onChange={(e) => setNewRatingsCount(parseInt(e.target.value) || 0)}
              ...
              <div className="pt-2 border-t border-gray-100">
                ...
                <div className="text-2xl font-bold text-gray-800">
                  {salesEstimate.min} ~ {salesEstimate.max}
                </div>
              </div>
            </div>
          </div>

          {/* 2. Store Feedback Estimator */}
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2 text-gray-800">
             ...
             <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 mb-4">
              ...
              <div className="flex items-center gap-2">
                <input 
                  type="number" 
                  step="0.1"
                  value={feedbackRateMin}
                  onChange={(e) => setFeedbackRateMin(parseFloat(e.target.value))}
                  className="w-full px-2 py-1 text-sm border border-blue-200 rounded text-center"
                />
                <span className="text-gray-400">-</span>
                <input 
                  type="number" 
                  step="0.1"
                  value={feedbackRateMax}
                  onChange={(e) => setFeedbackRateMax(parseFloat(e.target.value))}
                  className="w-full px-2 py-1 text-sm border border-blue-200 rounded text-center"
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="number"
                  value={feedbackCount}
                  onChange={(e) => setFeedbackCount(parseInt(e.target.value) || 0)}
              ...
              <div className="pt-2 border-t border-gray-100">
                ...
                <div className="text-2xl font-bold text-gray-800">
                  {storeSalesEstimate.min} ~ {storeSalesEstimate.max}
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-sm text-gray-600 leading-relaxed">
          <div>
            ...
            <p className="font-mono bg-gray-100 p-2 rounded mb-2 text-xs">
              公式：X = (T × N - S) / (5 - T)
            </p>
